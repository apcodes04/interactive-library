# Shared Design System — Extension Modules (51–73)

Paste this file into Antigravity's context alongside `00-global-spec.md` before every module prompt. This extension set (classical ML + advanced LLM topics, modules 51–73) reuses the exact same design system as the original 50-module set — same colors, same panels, same theme, same legend rules — so the two sets feel like one continuous product, not two different ones bolted together.

## Color theory: colors carry meaning, not just decoration

Every page uses the same five-color semantic mapping already established across modules 1–50. Once a user learns it once, they never have to relearn it, on either set.
...
Some modules show several simultaneous *items of the same kind* that a user needs to visually tell apart and track across stages — module 54's several Random Forest trees, module 61's sequence of weak learners, module 60's several candidate nearest-neighbors, module 66's SFT/reward-model/policy stages. Coloring all of them identically works but reads flat and wastes a real opportunity: giving each item its own consistent color, carried through every stage it appears in, makes the diagram both more visually engaging and easier to trace ("this coral tree's vote in the forest is the same tree highlighted earlier in the ensemble view").
...
- Treat this like a professional product's design system, not a demo: the goal is something a user would want to look at for 23 modules in a row, not something that merely "functions." Favor restraint over decoration — warmth and contrast do the work, not extra ornamentation.
...
**On the 7 multi-stage modules, the legend must be dynamic, not static — only list what's actually visible in the current stage, never the full set up front.** Showing all possible legend entries on stage 1, when only some of those colors exist anywhere on stage 1's card, is confusing rather than helpful — it describes elements the user can't see yet. Build a small per-stage lookup (an object of stage-index → array of legend-item keys, keyed the same way on desktop and mobile) and re-render the legend's contents inside the same `renderStage()` function that already updates the explain panel, so it updates in lockstep with whichever stage is current — reached by clicking, scrubbing, or auto-play, exactly like every other stage-dependent element. If a stage has no color-coded elements at all, hide the legend entirely for that stage (`display: none`) rather than leaving an empty box. The original course's `module-47-rag-pipeline.md` (in the `diagrams/` folder, not this one) is the worked reference example of a per-stage legend mapping — each of this set's own multi-stage module files spells out its own exact per-stage legend contents the same way.

## Multi-stage modules: stage scrubber + persistent clickable stages

Any module that shows more than one sequential stage or step — building a decision tree split by split (module 53), K-Means assign/update iterations (module 55), Gradient Boosting's sequential rounds of weak learners (module 61), k-fold Cross-Validation's fold-by-fold splits (module 64), the RLHF/DPO pipeline's SFT → reward model → policy stages (module 66), Speculative Decoding's draft-then-verify steps (module 72), and building then querying a Vector DB/ANN index (module 73) — must **never** be a single "Run" button that auto-plays through everything and then leaves the user with no way back in. That pattern was tried on the original course's RAG module first and failed: the explanation moved on before anyone could read it, and once the animation finished there was nothing left to interact with. Reference `prototypes/module-47-rag-pipeline/` in the original `diagrams/` folder as the proven implementation of this exact pattern.
