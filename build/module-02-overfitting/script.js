(function () {
  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    // Seeded random for reproducible 25 points
    let seed = 7;
    ...
    // Precomputed anchor table interpolation
    const anchors = [
    ...
    // Generate fitted curve path for a given degree
    function generateCurvePoints(deg) {
      ...
      for (let i = 0; i <= numSteps; i++) {
        ...
        if (deg === 1) {
          y = 15 + 0.15 * x; // linear underfit
        } else if (deg >= 8) {
          // add wiggles proportional to degree
    ...
    function update(deg) {
      ...
      // Line color based on test error / overfitting state
      let lineColor = 'var(--success)';
      let statusText = '';
      if (deg <= 2) {
        lineColor = 'var(--primary)';
        statusText = `Degree ${deg}: High train and test errors — model is Underfitting (too simple).`;
      } else if (deg <= 6) {
        lineColor = 'var(--success)';
        statusText = `Degree ${deg}: Sweet spot! Both train and test errors are low and balanced.`;
      } else {
        lineColor = 'var(--danger)';
        statusText = `Degree ${deg}: Overfitting! Train error falls (${errs.trainErr.toFixed(2)}), but test error rises (${errs.testErr.toFixed(2)}) as the curve wiggles.`;
      }
