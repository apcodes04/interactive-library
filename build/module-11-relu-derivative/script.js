(function () {
  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    // Build Chart 1: f(x) = max(0, x)
    const w = 320, h = 200, margin = { top: 20, right: 20, bottom: 30, left: 35 };
    ...
    svgFx.append('g').attr('transform', `translate(${xS(0)},0)`).call(d3.axisLeft(yS1).ticks(5));
    ...
    // Open circle gap at (0, 0) and (0, 1) on f'(x)
    svgFxP.append('circle').attr('cx', xS(0)).attr('cy', yS2(0)).attr('r', 4).attr('fill', 'var(--bg)').attr('stroke', 'var(--danger)').attr('stroke-width', 2);
    svgFxP.append('circle').attr('cx', xS(0)).attr('cy', yS2(1)).attr('r', 4).attr('fill', 'var(--bg)').attr('stroke', 'var(--danger)').attr('stroke-width', 2);

    // Tracking dots
    const dot1 = svgFx.append('circle').attr('r', 6).attr('fill', 'var(--active)');
    const dot2 = svgFxP.append('circle').attr('r', 6).attr('fill', 'var(--active)');
    ...
    function update(x) {
      ...
      if (Math.abs(x) < 0.05) {
        explain.textContent = `At x = 0: Discontinuity corner! The derivative is mathematically undefined because the left limit is 0 and right limit is 1.`;
      } else if (x < 0) {
        explain.textContent = `At x = ${x.toFixed(1)} < 0: f(x) = 0.00. Gradient is 0.0 (inactive neuron region / potential dying ReLU).`;
      } else {
        explain.textContent = `At x = ${x.toFixed(1)} > 0: f(x) = ${fx.toFixed(2)}. Gradient is 1.0 (active neuron region / gradient flows perfectly without diminishing).`;
      }
