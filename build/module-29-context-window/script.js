...
for (let i = 1; i <= numTokens; i++) {
    ...
    // Only text every 5th or edge to save space
...
function updateHeatmap() {
    ...
    // Heatmap should show high reliability at edges of current window, low in middle
    ...
    // We can use a linear gradient: frozen outside, success at edges, danger at middle
...
function onPointerMove(e) {
    ...
    if (isDragging) {
        ...
        if (resizeSide === 'left') {
            ...
            if (currentWidthPercent < 5) {
                currentLeftPercent = startLeft + startWidth - 5;
                currentWidthPercent = 5;
            }
            ...
            if (currentWidthPercent < 5) currentWidthPercent = 5;
        }
    }
    updateView();
}
...
windowOverlay.addEventListener('touchstart', (e) => onPointerDown(e, 'drag'), {passive: false});

document.getElementById('handle-left').addEventListener('mousedown', (e) => { e.stopPropagation(); onPointerDown(e, 'resize', 'left'); });
document.getElementById('handle-left').addEventListener('touchstart', (e) => { e.stopPropagation(); onPointerDown(e, 'resize', 'left'); }, {passive: false});

document.getElementById('handle-right').addEventListener('mousedown', (e) => { e.stopPropagation(); onPointerDown(e, 'resize', 'right'); });
document.getElementById('handle-right').addEventListener('touchstart', (e) => { e.stopPropagation(); onPointerDown(e, 'resize', 'right'); }, {passive: false});
