(function () {
  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    const w = 680, h = 140, margin = { top: 20, right: 30, bottom: 30, left: 40 };
    ...
    const xS = d3.scaleBand().domain(labels).range([margin.left, w - margin.right]).padding(0.4);
    ...
    svgLogits.append('g').attr('transform', `translate(${margin.left},0)`).call(d3.axisLeft(ySLogits).ticks(5));

    svgProbs.append('g').attr('transform', `translate(0,${h - margin.bottom})`).call(d3.axisBottom(xS));
    svgProbs.append('g').attr('transform', `translate(${margin.left},0)`).call(d3.axisLeft(ySProbs).ticks(4).tickFormat(d => `${d}%`));
    ...
    function update() {
      ...
      // Render Labels on Probs
      const probTexts = svgProbs.selectAll('.text-prob').data(probs);
      probTexts.enter().append('text').attr('class', 'text-prob')
        .merge(probTexts)
        .attr('x', (d, i) => xS(labels[i]) + xS.bandwidth() / 2)
        .attr('y', d => ySProbs(d) - 4)
      ...
      document.getElementById('explainPanel').textContent =
        `Top probability class: ${labels[maxIdx]} at ${probs[maxIdx].toFixed(1)}% (Logit: ${logits[maxIdx].toFixed(1)}). Exponential scaling amplifies the largest logit while forcing total sum = 100%.`;
