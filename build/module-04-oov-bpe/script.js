...
function renderDiagram(animate) {
  ...
  currentTokens.forEach(token => {
    ...
    // If animate is true, we could figure out which token is newly merged.
    // For simplicity, any token length > 1 might flash, or we just flash all if animated and step > 0
    if (animate && currentStep > 0) {
      const prevTokens = states[currentStep - 1];
      // A token is "newly merged" if it wasn't in the previous array
