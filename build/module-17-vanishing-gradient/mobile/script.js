(function () {
  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    function setLayer(targetL) {
      ...
      legend.innerHTML = `
        <div class="legend-item"><span class="legend-dot" style="background:${rnnCur < 30 ? 'var(--danger)' : 'var(--primary)'}"></span> RNN: ${rnnCur.toFixed(1)}%</div>
        <div class="legend-item"><span class="legend-dot" style="background:var(--success)"></span> Transformer: ${transCur.toFixed(1)}%</div>
      `;

      document.getElementById('explainPanel').textContent =
        `L${targetL}: RNN decaying (${rnnCur.toFixed(1)}%). Transformer residual preserves (${transCur.toFixed(1)}%).`;
