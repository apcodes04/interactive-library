...
slider.addEventListener('input', () => {
    ...
    if (t < 0.5) explain.innerHTML = `Temperature is low (${t.toFixed(2)}). The distribution becomes sharper, making the most likely token heavily favored.`;
    else if (t > 1.5) explain.innerHTML = `Temperature is high (${t.toFixed(2)}). The distribution becomes flatter, making all tokens more equally likely.`;
    else explain.innerHTML = `Temperature is ${t.toFixed(2)}. This gives a balanced mix of predictability and randomness.`;
});
...
btnSample.addEventListener('click', () => {
    ...
    explain.innerHTML = `Sampled token: <strong>"${labels[selectedIdx]}"</strong> based on the T=${t.toFixed(2)} probability distribution.`;
