...
setTimeout(() => {
  ...
  function encodeNext() {
    ...
    // Animate token from inBox to contextVector
    ...
    const startX = inRect.left - diagRect.left + inRect.width/2 - 10;
    const startY = inRect.top - diagRect.top + inRect.height/2 - 10;
    const endX = cRect.left - diagRect.left + cRect.width/2 - 10;
    const endY = cRect.top - diagRect.top + cRect.height/2 - 10;
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        pulseCircle();
        encodeStep++;
        setTimeout(encodeNext, 200); // Wait before next word
      }, 400); // match CSS transition
    }, 50); // slight delay to allow transform reset
  }
  encodeNext();
}, 500);
