# Shared Design System — All 50 Interactive Diagrams

Paste this file into Antigravity's context alongside `00-global-spec.md` before every module prompt. It exists so 50 pages built separately, possibly on different days, still feel like one course rather than 50 unrelated demos — and so the interactions feel satisfying to use, not just functionally correct.

## Color theory: colors carry meaning, not just decoration

Every page uses the same five-color semantic mapping. Once a user learns it on module 1, they never have to relearn it on module 50.
...
Some modules show several simultaneous *items of the same kind* that a user needs to visually tell apart and track across stages — module 47's 3 retrievable chunks (Code gen / Chat / Creative), module 24's multiple attention heads, module 36's 8 experts, module 4's BPE merge candidates, module 32's several candidate next-tokens. Coloring all of them identically (as module 47's chunks originally were — all solid indigo) works but reads flat and wastes a real opportunity: giving each item its own consistent color, carried through every stage it appears in, makes the diagram both more visually engaging and easier to trace ("this teal dot in the embedding space is the same 'Code gen' chunk as this teal-bordered box in the final prompt").
...
2. **Never assign a categorical color that collides with a semantic color active in the same view.** Module 47 is the concrete example: green is off-limits there for a chunk category because the same view already uses green for the "retrieved" highlight ring — reusing it for a chunk's base color would blur the two meanings. Pick from the table above, skipping any hue that would echo a semantic color already doing work on that page.
3. Carry the same item's color through every stage it appears in — the embedding dot, the chunk box, any later highlight — rather than assigning colors fresh at each stage.
4. Cap it at however many items the module actually needs (3 for module 47's chunks, up to 6 from this table) — don't add categorical color to items that don't need individual tracking (module 47's 10 decoy chunks correctly stay uniform `--frozen` gray; giving them individual colors would undermine the "these are irrelevant noise" signal).
5. If a module needs more than 6 simultaneous items (e.g. module 36's 8 experts), fall back to the tint/shade approach — two or three of the categorical hues each in 2-3 lightness steps — rather than inventing a 7th+ hue.
...
- Treat this like a professional product's design system, not a demo: the goal is something a user would want to look at for 50 modules in a row, not something that merely "functions." Favor restraint over decoration — warmth and contrast do the work, not extra ornamentation.
...
- No animation in this entire set should exceed ~1.5 seconds for a single step. Multi-stage pipelines (modules 23, 31, 47) chain several short steps rather than using one long animation.
...
**On the 8 multi-stage modules, the legend must be dynamic, not static — only list what's actually visible in the current stage, never the full set up front.** Showing all 6 possible legend entries (say) on stage 1, when only 3 of those colors exist anywhere on stage 1's card, is confusing rather than helpful — it describes elements the user can't see yet. Build a small per-stage lookup (an object of stage-index → array of legend-item keys, keyed the same way on desktop and mobile) and re-render the legend's contents inside the same `renderStage()` function that already updates the explain panel, so it updates in lockstep with whichever stage is current — reached by clicking, scrubbing, or auto-play, exactly like every other stage-dependent element. If a stage has no color-coded elements at all, hide the legend entirely for that stage (`display: none`) rather than leaving an empty box. Module 47 is the reference implementation: stage 1 (Chunks) shows only the 3 chunk-category entries; stage 2 (Embeddings) adds the decoy entry; stage 5 (Nearest match) is the only stage that additionally shows the query and retrieved entries, since those elements only exist from that stage onward; stage 6 (Prompt) drops back to only the 2 chunks that were actually retrieved, not all 3.

## Multi-stage modules: stage scrubber + persistent clickable stages

Any module that shows more than one sequential stage or step — the RAG pipeline (module 47), computing attention scores (module 23), beam search level-by-level (module 31), vanishing gradient layer-by-layer (module 17), BPE merges (module 4), the seq2seq decode-one-word-at-a-time animation (module 8), the embedding gradient nudge steps (module 15), and the chain rule forward/backward walkthrough (module 12) — must **never** be a single "Run" button that auto-plays through everything and then leaves the user with no way back in. That pattern was tried on the RAG module first and failed: the explanation moved on before anyone could read it, and once the animation finished there was nothing left to interact with.
